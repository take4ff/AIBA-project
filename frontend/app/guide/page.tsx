  ...
  return (
    <main className="container">
      <header className="header">
        <h1><BookOpen size={24} strokeWidth={2.2} style={{ verticalAlign: "-0.15em", color: "var(--accent)" }} /> スコア定義</h1>
      ...
      <section className="layer def-section">
        <h2 className="layer-title"><ConceptIcon name="themes" size={18} /> このツールの使い方（コア・サテライト戦略）</h2>
        <p>
...
モメンタム = (MA位置 + RSI勢い + 直近上昇) / 3`}</pre>
        <p className="guide-note">ランキングでは65以上で「<ConceptIcon name="momentum" size={12} /> 順張り」タグ、銘柄詳細では数値と基調（上昇/中立/下降）を表示。</p>
      </section>

      {/* 傾き・乖離 */}
      <section className="layer def-section">
        <h2 className="layer-title">センチメント傾き（↑↓）と <ConceptIcon name="divergence" size={18} /> 乖離</h2>
        <ul className="def-list">
          <li><strong>センチメント傾き ↑↓</strong>：直近のセンチメント変化。↑=熱量上昇（将来の伸びの先行サイン）、↓=減速、→=横ばい。</li>
          <li><ConceptIcon name="divergence" /> <strong>乖離</strong>：センチメント（先行指標）が上昇しているのに株価がまだ追いついていない状態（傾き&gt;1 かつ 株価変化&lt;2%）。本システムが狙う<strong>仕込み好機</strong>のサイン。</li>
        </ul>
      </section>
      ...
      <section className="layer def-section">
        ...
        <ul className="def-list">
          ...
          <li><strong>一目均衡表</strong>：転換線(9)・基準線(26)・先行スパンA/B(雲)。株価が<strong>雲の上＝強気・雲の下＝弱気</strong>、転換線が基準線を上抜け＝好転。終値ベースの近似。</li>
          <li><strong>テクニカル総合判定（買い・売り目安の網羅）</strong>：移動平均(25/200)・MACD・一目均衡表（<strong>トレンド系＝順張りの方向</strong>）と RSI・ボリンジャー・ストキャスティクス（<strong>オシレーター系＝売られすぎ/買われすぎ</strong>）を各々 買い/売り/中立 に評価し、総合の傾き（買い寄り〜売り寄り）を提示。銘柄詳細・ポートフォリオ詳細に一覧表示。</li>
          <li><strong>購入目安</strong>：妥当値=25日MA／押し目買い目安=−1σ／下値支持=60日安値。</li>
          <li><strong>下方リスク・下値メド</strong>：「これ以上下がりにくい目安」を複数の支持で提示。<strong>下値メド=52週安値</strong>（1年で最も売られた水準・下落余地%付き）、<strong>近い支持=直近60日安値と−2σの高い方</strong>。あわせて<strong>年率ボラ・1年の最大下落率</strong>から<strong>値動きの安定度（下方耐性）</strong>を高/中/低で表示。さらに黒字・割安なら事業面でも下値は相対的に堅い旨を補足。<strong>いずれも目安で割れることもある＝「絶対の底」ではない</strong>。</li>
        </ul>
      </section>
