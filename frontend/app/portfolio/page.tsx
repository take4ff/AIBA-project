...
import {
  ...
  getHoldings, getTickerData, addHolding, updateHolding, deleteHolding,
...
const EMPTY = { ticker: "", name: "", currency: "JPY" as "JPY" | "USD", avg_cost: "" };
...
export default function PortfolioPage() {
  ...
  const [edit, setEdit] = useState<{ ticker: string; name: string; avg_cost: string } | null>(null);

  const reload = useCallback(async () => {
    const hs = await getHoldings();
    setHoldings(hs);
    const { metrics, funds } = await getTickerData(hs.map((h) => h.ticker));
    setMetrics(metrics);
    setFunds(funds);
  }, []);
  ...
  async function onAdd(e: React.FormEvent) {
    ...
    const msg = await addHolding({
      ...
      avg_cost: form.avg_cost ? Number(form.avg_cost) : null, shares: null,
    });
    if (msg) setErr(msg);
          ...
          {holdings.length === 0 ? (
            ...
            <div className="table-scroll" style={{ marginTop: 16 }}>
              <table className="table">
                <thead><tr>
                  <th>銘柄</th><th className="num">取得単価</th><th className="num">現在値</th>
                      ...
                      <tr key={h.ticker}>
                        ...
                        <td>
                          {editing ? (
                            ...
                            <>
                              <button className="authbar-btn" onClick={() => setEdit({ ticker: h.ticker, name: h.name ?? "", avg_cost: h.avg_cost?.toString() ?? "" })}>編集</button>{" "}
