...
export interface EquityPoint {
  date: string;
  buy: number;          // 買い判定(AIBA≥60)を毎月入れ替えた場合の資産
  buy70?: number;       // 厳格買い(AIBA≥70)
    ...
    <div className="chart-wrap" style={{ paddingTop: 8 }}>
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={data} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
          ...
          <Line type="monotone" dataKey="buy" name="AIBA買い(≥60)を毎月入替" stroke="#15a34a" strokeWidth={2.4} dot={false} />
          <Line type="monotone" dataKey="buy70" name="AIBA厳格買い(≥70)" stroke="#ea580c" strokeWidth={2.4} dot={false} connectNulls />
      ...
      <p style={{ color: "#71767f", fontSize: 12, marginTop: 4, textAlign: "center" }}>
        100スタート。緑＝買い判定(AIBA≥60)を毎月入替（買い無しの月は現金）、橙＝<strong>厳格買い(AIBA≥70)</strong>でさらに絞り込み。
        灰＝<strong>監視ユニバース（全テーマのETF＋個別株）の等ウェイト平均</strong>、青破線＝<strong>全世界株(ACWI)放置</strong>。
        重複・取引コスト未考慮の概算。
      </p>
    </div>
  );
