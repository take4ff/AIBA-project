...
- [x] **Google Trends** をセンチメント第4指標に追加（検索関心の増加率。取得失敗時は平均から除外）。**特許は保留**（PatentsView無料APIが実質終了・要APIキー）
- [ ] バックテスト結果のUI表示、乖離アラートの LINE/メール対応
- [x] **ランキングにスコアと並べて最新株価を表示**（株価列を追加。地域別に円/ドル）
- [ ] **UI/UXデザインの改善**（進行中）
- [x] **株価の購入目安**：銘柄詳細に妥当値(25MA)・押し目買い目安(−1σ)・下値支持(60日安値)を表示＋チャートに押し目目安の水平線
- [ ] 1ヶ月ごとにAIBAスコア等記録して、1,3,6ヶ月ごとにスコアが正しかったか評価、可視化
- [ ] 新技術・トピック用のページ
