...
- [ ] **健康度レーダー**（銘柄詳細）：割安/成長/テクニカル/モメンタム等を5角形表示（Simply Wall St風・既存スコア活用で軽い）★おすすめ
- [ ] **フェアバリュー（相対PER）**：テーマ内ピアの予想PER中央値 × 予想EPS で理論株価→「◯%割安/割高」
- [ ] **一行ナラティブ（自動要約）**：テーマ＋AIBA＋ファンダ＋解釈を1文で要約
- [ ] **カスタムスクリーナー**：スコア/層/地域/PER等の多条件フィルタ（Pickupの発展）
